...
import { AgentConfigInitializerInput, ExistingAgentConfig } from "./dto.js";
...
These two lines are **mandatory** and must appear first, each on its own line.`,
  ...
  .section({
    title: {
      text: "CREATE_AGENT_CONFIG — Rules (numbered for clarity)",
      level: 3,
    },
...
7. **Uniqueness guard** – If the proposed \`agent_type\` already exists, abort and return \`AGENT_CONFIG_UNAVAILABLE\`.`,
  ...
  .section({
    title: {
      text: "UPDATE_AGENT_CONFIG — Rules (numbered for clarity)",
      level: 3,
    },
...
6. **Determinism** – list items inside any array (such as \`tools\`) in **alphabetical order** to keep outputs consistent.`,
  ...
  .section({
    title: {
      text: "SELECT_AGENT_CONFIG — Rules (numbered for clarity)",
      level: 3,
    },
...
No extra whitespace, keys, or commentary beyond this structure.`,
  ...
  .section({
    title: {
      text: "AGENT_CONFIG_UNAVAILABLE — Rules (numbered for clarity)",
      level: 3,
    },
    newLines: {
      contentEnd: 0,
